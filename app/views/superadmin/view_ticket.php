    ...
    <div class="col-12">
        <div class="card" style="max-width: 100%;">
            <div class="card-header" style="background-color: #f8f9fa; border-bottom: 1px solid #dee2e6; padding: 15px; display: flex; justify-content: space-between; align-items: center;">
                <h3 class="card-title" style="margin: 0;">Ticket #<?php echo htmlspecialchars($data['ticket']->id); ?> (Tutor: <?php echo htmlspecialchars($data['ticket']->username); ?>)</h3>
                <a href="<?php echo URLROOT; ?>/superadmin/tickets" style="color: #64748b; text-decoration: none; font-weight: 500;">&larr; Back to Tickets</a>
            </div>
            <div class="card-body" style="padding: 20px;">
                ...
                <?php else: ?>
                    <div style="display: flex; flex-direction: column; gap: 15px; margin-bottom: 20px;">
                        <?php foreach ($replies as $reply): ?>
                            <?php $isAdmin = ($reply['userID'] == $_SESSION['user_id']); ?>
                            <div style="background: <?php echo $isAdmin ? '#e0f2fe' : '#f1f5f9'; ?>; padding: 12px 15px; border-radius: 6px; border: 1px solid <?php echo $isAdmin ? '#bae6fd' : '#e2e8f0'; ?>;">
                                <div style="font-size: 0.85rem; font-weight: 600; color: <?php echo $isAdmin ? '#0284c7' : '#475569'; ?>; margin-bottom: 5px;">
                                    <?php echo $isAdmin ? 'Me (Admin)' : 'Tutor'; ?> <span style="font-weight: 400; color: #94a3b8; font-size: 0.75rem; margin-left: 10px;"><?php echo date('M d, Y h:i A', strtotime($reply['timestamp'])); ?></span>
                                </div>
                ...
                <?php if ($data['ticket']->status !== 'closed'): ?>
                    <form action="<?php echo URLROOT; ?>/superadmin/viewTicket/<?php echo $data['ticket']->id; ?>" method="POST" style="margin-top: 20px; border-top: 1px solid #e2e8f0; padding-top: 20px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                            <button type="submit" name="close_ticket" value="1" class="btn btn-warning" formnovalidate style="background-color:#f59e0b; color: white; padding: 8px 16px; border: none; border-radius: 4px; font-weight: 600; cursor: pointer; width: auto; min-width: 0;">Close Ticket</button>
