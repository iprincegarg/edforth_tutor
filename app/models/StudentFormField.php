...
class StudentFormField {
    ...
    public function getFieldsWithDetails() {
        $this->db->query('
            SELECT f.*, s.sectionName
            FROM student_form_fields f
            JOIN student_sections s ON f.section_id = s.id
    ...
    public function getFieldById($id) {
        $this->db->query('SELECT * FROM student_form_fields WHERE id = :id');
        $this->db->bind(':id', $id);
        return $this->db->single();
    }
    
    public function countFileFields() {
        $this->db->query('SELECT COUNT(*) as count FROM student_form_fields WHERE field_type = "file"');
        $row = $this->db->single();
        return $row->count;
    }
    ...
    public function addField($data) {
        $this->db->query('
            ...
            (section_id, field_name, field_type, char_limit, placeholder_text, field_values, is_required, show_on_form, show_to_user) 
            VALUES 
            (:section_id, :field_name, :field_type, :char_limit, :placeholder_text, :field_values, :is_required, :show_on_form, :show_to_user)
        ');
    ...
    public function deleteField($id) {
        $this->db->query('DELETE FROM student_form_fields WHERE id = :id');
        $this->db->bind(':id', $id);
        return $this->db->execute();
    }

    public function updateSequence($id, $sequence) {
        $this->db->query('UPDATE student_form_fields SET sequence_id = :sequence WHERE id = :id');
        $this->db->bind(':id', $id);
